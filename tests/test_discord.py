...
def test_send_open_alert_includes_showtime_link_buttons(monkeypatch):
    """회차별 링크 버튼을 with_components=true 로 전송한다."""
    monkeypatch.setenv("DISCORD_WEBHOOK_URL", "https://discord.test/hook")
    resp = MagicMock(status_code=204)
    post = MagicMock(return_value=resp)
    showtimes = [{"start": "1800", "screen": "2관 (Laser)", "free_seats": "34"}]

    discord.send_open_alert(_watch(), Settings(), showtimes, post=post)

    url = post.call_args[0][0]
    payload = post.call_args[1]["json"]
    assert "with_components=true" in url
    buttons = payload["components"][0]["components"]
    assert buttons[0]["style"] == 5  # 링크 버튼
    assert "18:00" in buttons[0]["label"]
    assert buttons[0]["url"].startswith("https://cgv.co.kr/cnm/movieBook/movie?")


def test_send_open_alert_without_showtimes_has_no_components(monkeypatch):
    monkeypatch.setenv("DISCORD_WEBHOOK_URL", "https://discord.test/hook")
    resp = MagicMock(status_code=204)
    post = MagicMock(return_value=resp)
